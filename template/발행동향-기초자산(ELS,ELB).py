...
value1 = input("상품유형을 입력하세요 ex) 전체, ELS, ELB중 1개 입력 : ")
...
payload = "<reqParam action=\"bassetTop10List\" task=\"ksd.safe.bip.cnts.DerivCombi.process.DeriELSPTask\"><SECN_TPCD value=\""+num1+"\"/><DERI_INFO value=\"2\"/><STD_DT1 value=\""+date1+"\"/><STD_DT2 value=\""+date2+"\"/><START_PAGE value=\"1\"/><END_PAGE value=\"10\"/></reqParam>"
headers = {
  ...
  'submissionid': 'submission_bassetTop10List',
...
wb = load_workbook(filename = '/Users/yerimstar/PycharmProjects/예탁원/템플릿/예탁원_발행동향(기초자산)_템플릿.xlsm', data_only=True)
ws = wb[wb.sheetnames[0]]
length = int(jsonText["vector"]["@result"])

if length > 1:
    for i in range(0, length):
        value = str(jsonText["vector"]["data"][i]["result"]["STND_BASSET_NM"]["@value"])
    ...
    wb.save("/Users/yerimstar/PycharmProjects/예탁원/결과물/예탁원_발행동향_Top10기초자산_" + value1 + "(" + date1 + "-" + date2 + ").xlsx")
else:
    print("값이 없습니다.")
